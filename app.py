...
@app.route("/api/acq/geocode/suggest")
@login_required
def api_acq_geocode_suggest():
    ...
    try:
        return jsonify({"suggestions": acq_gis.geocode_suggest(q)})
    except Exception as e:
        print(f"[acq] geocode suggest failed: {e}", flush=True)
        return jsonify({"suggestions": []})


@app.route("/api/acq/geocode")
@login_required
def api_acq_geocode():
    guard = _acq_guard()
    if guard:
...
@app.route("/api/acq/load-layer/<key>")
@login_required
...
def api_acq_parcels():
    ...
    try:
        return jsonify(acq_gis.arcgis_query(
            acq_gis.ENDPOINTS["parcels"], bbox=(minx, miny, maxx, maxy),
...
@app.route("/api/acq/owner-dossier")
@login_required
def api_acq_owner_dossier():
    ...
    tokens = acq_parcels._owner_tokens(name)
    used_fuzzy = False
    if exact_only or len(tokens) < 2:
        result = acq_parcels.find_parcels_by_owner(name, exact=True, limit=500)
    else:
        result = acq_parcels.find_parcels_by_owner(name, exact=False, limit=500)
        used_fuzzy = True
...
@app.route("/api/acq/parcel-detail/<prop_id>")
@login_required
def api_acq_parcel_detail(prop_id):
    ...
    if guard:
        return guard
    try:
        return jsonify(acq_gis.parcel_detail(prop_id))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
@app.route("/api/acq/projects", methods=["GET"])
@login_required
...
@app.route("/api/acq/projects", methods=["POST"])
@login_required
...
@app.route("/api/acq/projects/<pid>", methods=["GET"])
@login_required
...
@app.route("/api/acq/projects/<pid>", methods=["PATCH"])
@login_required
...
@app.route("/api/acq/projects/<pid>", methods=["DELETE"])
@login_required
...
@app.route("/api/acq/tract-pins", methods=["GET", "POST"])
@login_required
...
@app.route("/api/acq/tract-pins/<pin_id>", methods=["DELETE"])
@login_required
...
@app.route("/api/acq/folders", methods=["GET", "POST"])
@login_required
def api_acq_folders():
    guard = _acq_guard()
    if guard:
...
@app.route("/api/acq/notes/by-props", methods=["POST"])
@login_required
...
@app.route("/api/acq/notes/<prop_id>", methods=["GET", "POST"])
@login_required
...
@app.route("/api/acq/notes/id/<note_id>", methods=["DELETE"])
@login_required
...
@app.route("/api/acq/search", methods=["POST"])
@login_required
def api_acq_search():
    ...
    source = "cache"
    try:
        coverage = acq_parcels.coverage_for_polygon(buf)
        cached = acq_parcels.is_fully_cached(coverage)
    except Exception as e:
    ...
    try:
        if cached:
            features = acq_parcels.query_parcels_in_polygon(buf, min_acres, max_acres)
        else:
            source = "live"
            fc = acq_gis.arcgis_query(
                acq_gis.ENDPOINTS["parcels"], geometry_polygon=buf,
                out_fields="Prop_ID,OWNER_NAME,SITUS_ADDR,MAIL_ADDR,LEGAL_AREA,GIS_AREA,LEGAL_DESC",
    ...
    for fn, label in ((acq_gis.hcad_live_overlay, "hcad"),
                      (acq_gis.mcad_live_overlay, "mcad")):
        try:
            fn(fc)
        except Exception as e:
...
def api_acq_cache_status():
    ...
    if guard:
        return guard
    try:
        return jsonify(acq_parcels.cache_status())
    except Exception as e:
